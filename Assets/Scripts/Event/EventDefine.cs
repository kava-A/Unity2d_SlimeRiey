...
public static class EventDefine
{
    ...
    //定义boss生成事件
    public static event Action<Transform> OnEnemySpawnEvent;
    public static void CallEnemySpawnEvent(Transform position)
    {
        if (OnEnemySpawnEvent == null)
        {
            Debug.LogWarning("OnEnemySpawnEvent 事件没有订阅者");
            return;
        }
        OnEnemySpawnEvent.Invoke(position); // 执行事件
    }
}
